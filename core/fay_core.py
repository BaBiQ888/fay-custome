...
class FeiFei:
    def __init__(self):
        ...
        self.W = np.array([0.0, 0.6, 0.1, 0.7, 0.3, 0.0,
                          0.0, 0.0]).reshape(-1, 1)  # 适应模型变量矩阵
    ...
    def __remove_emojis(self, text):
        ...
        emoji_pattern = re.compile(
            ...
            # 补充符号和象形文字 (Supplemental Symbols and Pictographs)
            "\U0001F900-\U0001F9FF"
            # 扩展A符号和象形文字 (Symbols and Pictographs Extended-A)
            "\U0001FA70-\U0001FAFF"
    ...
    def __process_qa_stream(self, text, username):
        ...
        success = processor.process_stream_text(
            text, username, is_qa=True, session_type="qa")
        ...
        if self.__running:
            try:
                ...
                if index == 1:  # 语音文字交互
                    # 记录用户问题,方便obs等调用
                    self.write_to_file(
                        "./logs", "asr_result.txt",  interact.data["msg"])

                    # 同步用户问题到数字人
                    if wsa_server.get_instance().is_connected(username):
                        content = {'Topic': 'human', 'Data': {
                            'Key': 'question', 'Value': interact.data["msg"]}, 'Username': interact.data.get("user")}
                    ...
                    if wsa_server.get_web_instance().is_connected(username):
                        wsa_server.get_web_instance().add_cmd({"panelReply": {
                            "type": "member", "content": interact.data["msg"], "username": username, "uid": uid, "id": content_id}, "Username": username})

                    # 确定是否命中q&a
                    answer, type = self.__get_answer(
                        interact.interleaver, interact.data["msg"])
                    ...
                    if answer is None or type != "qa":
                        if wsa_server.get_web_instance().is_connected(username):
                            wsa_server.get_web_instance().add_cmd(
                                {"panelMsg": "思考中...", "Username": username, 'robot': f'{cfg.fay_url}/robot/Thinking.jpg'})
                        if wsa_server.get_instance().is_connected(username):
                            content = {'Topic': 'human', 'Data': {'Key': 'log', 'Value': "思考中..."},
                                       'Username': username, 'robot': f'{cfg.fay_url}/robot/Thinking.jpg'}
                            wsa_server.get_instance().add_cmd(content)
                        text = nlp_cognitive_stream.question(
                            interact.data["msg"], username, interact.data.get("observation", None))
        ...
        if interact.interact_type == 1:
            try:
                if cfg.ltp_mode == "cemotion":
                    result = nlp_cemotion.get_sentiment(
                        self.cemotion, interact.data["msg"])
                    ...
                    if str(cfg.baidu_emotion_api_key) == '' or str(cfg.baidu_emotion_app_id) == '' or str(cfg.baidu_emotion_secret_key) == '':
                        ...
                        result = int(baidu_emotion.get_sentiment(
                            interact.data["msg"]))
                        chat_perception = perception["chat"]
                        if result >= 2:
                            self.mood = self.mood + (chat_perception / 150.0)
    ...
    def say(self, interact, text, type=""):  # TODO 对is_end及is_first的处理有问题
        try:
            ...
            self.__send_panel_message(
                text, interact.data.get('user'), uid, 0, type)
            ...
            if self.think_mode_users.get(uid, False) and is_start_think:
                if wsa_server.get_web_instance().is_connected(interact.data.get('user')):
                    wsa_server.get_web_instance().add_cmd({"panelMsg": "思考中...", "Username": interact.data.get(
                        'user'), 'robot': f'{cfg.fay_url}/robot/Thinking.jpg'})
                if wsa_server.get_instance().is_connected(interact.data.get("user")):
                    content = {'Topic': 'human', 'Data': {'Key': 'log', 'Value': "思考中..."}, 'Username': interact.data.get(
                        'user'), 'robot': f'{cfg.fay_url}/robot/Thinking.jpg'}
                    wsa_server.get_instance().add_cmd(content)
            ...
            if audio_url is not None:  # 透传音频下载
                file_name = 'sample-' + \
                    str(int(time.time() * 1000)) + audio_url[-4:]
                ...
                if text != None and text.replace("*", "").strip() != "":
                    ...
                    if filtered_text is not None and filtered_text.strip() != "":
                        util.printInfo(
                            1,  interact.data.get('user'), '合成音频...')
                        tm = time.time()
                        result = self.sp.to_sample(
                            filtered_text, self.__get_mood_voice())
                        util.printInfo(1,  interact.data.get("user"), "合成音频完成. 耗时: {} ms 文件:{}".format(
                            math.floor((time.time() - tm) * 1000), result))
            else:
                if is_end and wsa_server.get_web_instance().is_connected(interact.data.get('user')):
                    wsa_server.get_web_instance().add_cmd({"panelMsg": "", 'Username': interact.data.get(
                        'user'), 'robot': f'{cfg.fay_url}/robot/Normal.jpg'})

            if result is not None or is_first or is_end:
                if is_end:  # 如果结束标记，则延迟1秒处理,免得is end比前面的音频tts要快
                    time.sleep(1)
                MyThread(target=self.__process_output_audio,
                         args=[result, interact, text]).start()
                return result
    ...
    def __play_sound(self):
        ...
        while self.__running:
            ...
            if not self.sound_query.empty():  # 如果队列不为空则播放音频
                ...
                if wsa_server.get_web_instance().is_connected(interact.data.get('user')):
                    wsa_server.get_web_instance().add_cmd({"panelMsg": "播放中 ...", "Username": interact.data.get(
                        'user'), 'robot': f'{cfg.fay_url}/robot/Speaking.jpg'})
                ...
                if wsa_server.get_web_instance().is_connected(interact.data.get("user")):
                    wsa_server.get_web_instance().add_cmd(
                        {"panelMsg": "", 'Username': interact.data.get('user')})
    ...
    def __send_remote_device_audio(self, file_url, interact):
        ...
        for key, value in fay_booter.DeviceInputListenerDict.items():
            # 按username选择推送，booter.devicelistenerdice按用户名记录
            if value.username == interact.data.get("user") and value.isOutput:
                try:
                    value.deviceConnector.send(
                        b"\x00\x01\x02\x03\x04\x05\x06\x07\x08")  # 发送音频开始标志，同时也检查设备是否在线
                    ...
                    while data:
                        ...
                        time.sleep(0.0001)
                    value.deviceConnector.send(
                        b'\x08\x07\x06\x05\x04\x03\x02\x01\x00')  # 发送音频结束标志
                    util.printInfo(1, value.username,
                                   "远程音频发送完成：{}".format(total))
                except socket.error as serr:
                    util.printInfo(1, value.username,
                                   "远程音频输入输出设备已经断开：{}".format(key))
                    value.stop()
                    delkey = key
        if delkey:
    ...
    def __process_output_audio(self, file_url, interact, text):
        try:
            ...
            # 推送远程音频
            if file_url is not None:
                MyThread(target=self.__send_remote_device_audio,
                         args=[file_url, interact]).start()

            # 发送音频给数字人接口
            if file_url is not None and wsa_server.get_instance().is_connected(interact.data.get("user")):
                content = {'Topic': 'human', 'Data': {'Key': 'audio', 'Value': os.path.abspath(file_url), 'HttpValue': f'{cfg.fay_url}/audio/' + os.path.basename(file_url),  'Text': text, 'Time': audio_length, 'Type': interact.interleaver, 'IsFirst': 1 if interact.data.get(
                    "isfirst", False) else 0,  'IsEnd': 1 if interact.data.get("isend", False) else 0}, 'Username': interact.data.get('user'), 'robot': f'{cfg.fay_url}/robot/Speaking.jpg'}
                # 计算lips
                if platform.system() == "Windows":
                    try:
                        lip_sync_generator = LipSyncGenerator()
                        viseme_list = lip_sync_generator.generate_visemes(
                            os.path.abspath(file_url))
                        consolidated_visemes = lip_sync_generator.consolidate_visemes(
                            viseme_list)
                        content["Data"]["Lips"] = consolidated_visemes
                    except Exception as e:
                        print(e)
                        util.printInfo(
                            1, interact.data.get("user"),  "唇型数据生成失败")
                wsa_server.get_instance().add_cmd(content)
    ...
    def __process_text_output(self, text, username, uid):
        ...
        # 打印日志
        util.printInfo(1, username, '({}) {}'.format(
            self.__get_mood_voice(), text))
