...
try:
    ...
    # Python 3.13+ 中 audioop 被移除，使用替代实现
    audioop = None
import math
import time
import threading
...
# Python 3.13+ 兼容性：替代 audioop.rms 的函数


def _calculate_rms(data, width):
    """计算音频数据的RMS值，替代audioop.rms"""
    ...
    if width == 2:  # 16-bit audio
        ...
        format_char = 'h'  # 默认使用16-bit

    # 将字节数据转换为数值
    samples = struct.unpack(f'<{len(data)//width}{format_char}', data)
...
class Recorder:
    ...
    def save_buffer_to_file(self, buffer):
        temp_file = tempfile.NamedTemporaryFile(
            delete=False, suffix=".wav", dir="cache_data")
    ...
    def __waitingResult(self, iat: asrclient, audio_data):
        ...
        util.printInfo(1, self.username, "语音处理完成！ 耗时: {} ms".format(
            math.floor((time.time() - tm) * 1000)))
        if len(text) > 0:
            if cfg.config['source']['wake_word_enabled']:
                # 普通唤醒模式
                if cfg.config['source']['wake_word_type'] == 'common':
                    ...
                    if not self.wakeup_matched:
                        ...
                        if wake_up:
                            ...
                            if wsa_server.get_web_instance().is_connected(self.username):
                                wsa_server.get_web_instance().add_cmd(
                                    {"panelMsg": "唤醒成功！", "Username": self.username, 'robot': f'{cfg.fay_url}/robot/Listening.jpg'})
                            ...
                            state_manager.start_new_session(
                                self.username, "auto_play")

                            intt = interact.Interact("auto_play", 2, {
                                                     'user': self.username, 'text': "在呢，你说？", "isfirst": True, "isend": True})
                            ...
                            if wsa_server.get_web_instance().is_connected(self.username):
                                wsa_server.get_web_instance().add_cmd(
                                    {"panelMsg": "[!] 待唤醒！", "Username": self.username, 'robot': f'{cfg.fay_url}/robot/Normal.jpg'})
                            if wsa_server.get_instance().is_connected(self.username):
                                content = {'Topic': 'human', 'Data': {
                                    'Key': 'log', 'Value': "[!] 待唤醒！"}, 'Username': self.username, 'robot': f'{cfg.fay_url}/robot/Normal.jpg'}
                                wsa_server.get_instance().add_cmd(content)
                    else:
                        self.on_speaking(text)
                        self.processing = False
                        ...
                        self.timer = threading.Timer(
                            60, self.reset_wakeup_status)  # 重设计时器为60秒
                        self.timer.start()
    ...
    def __record(self):
        ...
        while self.__running:
            ...
            if history_percentage > self.__dynamic_threshold:
                self.__dynamic_threshold += (history_percentage -
                                             self.__dynamic_threshold) * 0.0025
            elif history_percentage < self.__dynamic_threshold:
                self.__dynamic_threshold += (history_percentage -
                                             self.__dynamic_threshold) * 1
                ...
                if percentage > self.__dynamic_threshold:
                    ...
                    if not self.__processing and not isSpeaking and time.time() - last_mute_time > _ATTACK:
                        ...
                        while not self.__aLiNls.started:
                            time.sleep(0.01)
                        # 当前data在下面会做发送，这里是发送激活前的音频数据，以免漏掉信息
                        for i in range(len(self.__history_data) - 1):
                            buf = self.__history_data[i]
                            audio_data_list.append(
                                self.__process_audio_data(buf, self.channels))
                            if self.ASRMode == "ali":
                                self.__aLiNls.send(self.__process_audio_data(
                                    buf, self.channels).tobytes())
                            else:
                                concatenated_audio.extend(
                                    self.__process_audio_data(buf, self.channels).tobytes())
                    ...
                    if isSpeaking:
                        if time.time() - last_speaking_time > _RELEASE:
                            ...
                            mono_data = self.__concatenate_audio_data(
                                audio_data_list)
                            self.__waitingResult(self.__aLiNls, mono_data)
                            self.__save_audio_to_wav(
                                mono_data, self.sample_rate, "cache_data/input.wav")
                            audio_data_list = []

                # 拾音中
                if isSpeaking:
                    audio_data_list.append(
                        self.__process_audio_data(data, self.channels))
                    if self.ASRMode == "ali":
                        self.__aLiNls.send(self.__process_audio_data(
                            data, self.channels).tobytes())
                    else:
                        concatenated_audio.extend(
    ...
    def __notify_listening_status(self):
        ...
        if self.__ws_notify_thread is None or not self.__ws_notify_thread.is_alive():
            self.__ws_notify_thread = threading.Thread(
                target=send_ws_notification)
