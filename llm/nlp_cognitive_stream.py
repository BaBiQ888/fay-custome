        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                for byte in raw_data:
                    char = chr(byte) if 32 <= byte <= 126 or byte in [
                        9, 10, 13] else None
                    if char:
                        current_text += char
                    else:
...
def search_knowledge_base(query, knowledge_base, max_results=3):
    ...
    for file_name, content in knowledge_base.items():
        ...
        if score > 0:
            ...
            # 取前几个最相关的句子
            relevant_sentences = [sent[0]
                                  for sent in matched_sentences[:5] if sent[0]]
    ...
    if agent.memory_stream and len(agent.memory_stream.seq_nodes) > 0:
        ...
        try:
            related_memories = agent.memory_stream.retrieve(
                ...
                # 权重：[时间近度权重recency_w, 相关性权重relevance_w, 重要性权重importance_w]
                hp=[0, 1, 0.5],
                stateless=False
            )
    ...
    try:
        ...
        if knowledge_base:
            knowledge_results = search_knowledge_base(
                content, knowledge_base, max_results=3)
...
你将参与日常问答、任务执行、工具调用以及角色扮演等多轮对话。请始终以符合以上人设的身份和语气与用户交流,回答要精简,不要超过60字。
...
"""
    ...
    if mcp_tools:
        ...
        for chunk in react_agent.stream(
            {"messages": messages}, {"configurable": {
                "thread_id": "tid{}".format(username)}}
        ):
            react_response_text = ""
            # 消息类型1：检测工具调用开始
        ...
        if accumulated_text:
            ...
            marked_text, _, _ = state_manager.prepare_sentence(
                username, accumulated_text, force_end=True)
            stream_manager.new_instance().write_sentence(username, marked_text)
        else:
        ...
        try:
            ...
            stream_manager.new_instance().write_sentence(
                username, "_<isfirst>" + error_message + "_<isend>")
    ...
    # 在单独线程中记忆对话内容
    MyThread(target=remember_conversation_thread, args=(
        username, content, full_response_text.split("</think>")[-1])).start()
        ...
        with save_lock:
            ...
            with agent_lock:
                ...
                for username, agent in agents.items():
                    ...
                    mem_base = os.path.join(base_dir, "memory")
                    memory_cleared_flag_file = os.path.join(
                        mem_base, ".memory_cleared")
                    if os.path.exists(memory_cleared_flag_file):
                    ...
                    try:
                        ...
                        # 重建id_to_node字典
                        agent.memory_stream.id_to_node = {
                            node.node_id: node for node in valid_nodes if hasattr(node, 'node_id')}
                    except Exception as e:
                    ...
                    try:
                        ...
                        try:
                            # 创建必要的目录
                            memory_stream_dir = os.path.join(
                                memory_dir, "memory_stream")
                            ...
                            with open(os.path.join(memory_stream_dir, "embeddings.json"), "w", encoding='utf-8') as f:
                                json.dump(agent.memory_stream.embeddings or {
                    ...
                    try:
                        ...
                        agent.scratch["current_time"] = datetime.datetime.now().strftime(
                            "%Y-%m-%d %H:%M:%S")
                    except Exception as e:
...
def _build_tool(tool_def: dict) -> StructuredTool:
    ...
    def _caller(**kwargs):
        ...
        try:
            resp = requests.post(
                f"http://127.0.0.1:5010/api/mcp/tools/{name}", json=kwargs, timeout=120)
            data = resp.json()
            if data.get("success"):
