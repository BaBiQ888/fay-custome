...
class RecorderListener(Recorder):
    ...
    def get_stream(self):
        try:
            ...
            self.channels = min(int(default_device.get(
                'maxInputChannels', 1)), 2)  # 最多使用2个通道
            # self.sample_rate = int(default_device.get('defaultSampleRate', 16000))

            util.printInfo(
                1, "系统", f"默认麦克风信息 - 采样率: {self.sample_rate}Hz, 通道数: {self.channels}")
    ...
    def on_speaking(self, text):
        ...
        if len(text) > 1:
            interact = Interact("socket", 1, {
                                "user": self.username, "msg": text, "socket": self.deviceConnector})
            util.printInfo(3, "(" + self.username + ")远程音频输入",
                           '{}'.format(interact.data["msg"]), time.time())
            feiFei.on_interact(interact)
...
def device_socket_keep_alive():
    ...
    while __running:
        ...
        for key, value in DeviceInputListenerDict.items():
            try:
                value.deviceConnector.send(
                    b'\xf0\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8')  # 发送心跳包
                if wsa_server.get_web_instance().is_connected(value.username):
                    wsa_server.get_web_instance().add_cmd(
                        {"remote_audio_connect": True, "Username": value.username})
        ...
        if delkey:
            ...
            if wsa_server.get_web_instance().is_connected(value.username):
                wsa_server.get_web_instance().add_cmd(
                    {"remote_audio_connect": False, "Username": value.username})
        time.sleep(10)
...
def accept_audio_device_output_connect():
    ...
    while __running:
        try:
            ...
            deviceInputListener = DeviceInputListener(
                deviceConnector, feiFei)  # 设备音频输入输出麦克风
            deviceInputListener.start()

            # 把DeviceInputListenner对象记录下来
            peername = str(deviceConnector.getpeername()[
                           0]) + ":" + str(deviceConnector.getpeername()[1])
            DeviceInputListenerDict[peername] = deviceInputListener
            util.log(1, "远程音频{}输入输出设备连接上：{}".format(
                len(DeviceInputListenerDict), addr))
        except Exception as e:
            pass
            ...
            if config_util.config['source']['automatic_player_status'] and config_util.config['source']['automatic_player_url'] is not None and get_fay_core().can_auto_play == True and (config_util.config["interact"]["playSound"] or wsa_server.get_instance().is_connected(user)):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        interact = Interact(
                            "auto_play", 2, {'user': user, 'text': response_text, 'audio': audio_url})
                        util.printInfo(1, user, '自动播报：{}，{}'.format(
                            response_text, audio_url), time.time())
                        feiFei.on_interact(interact)
                    else:
                        is_auto_server_error = True
                        get_fay_core().can_auto_play = True
                        util.printInfo(1, user, '请求自动播报服务器失败，错误代码是：{}'.format(
