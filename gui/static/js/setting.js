...
new Vue({
	...
	methods: {
		initFayService() {
			...
			this.fayService = new FayInterface(
				`${wsProtocol}://${wsHost}:10003`,
				this.host_url,
				this
			);
		...
		clearMemory() {
			this.$confirm(
				"清除记忆操作将删除Fay的所有对话记忆，清除后需要重启应用才能生效，确认继续吗?",
				"提示",
				{
					...
					fetch(`${this.host_url}/api/start-genagents`, {
						...
						.then((data) => {
							if (data.success) {
								// 弹出提示，显示克隆地址，不自动打开
								this.$alert(
									`决策分析页面已启动，请复制以下链接在新窗口中打开：<br><br><code style="background-color: #f5f5f5; padding: 5px; border-radius: 3px;">${data.url}</code>`,
									"克隆人格",
									{
										confirmButtonText: "确定",
