...
class FayInterface {
	...
	connectWebSocket() {
		...
		this.websocket.onclose = () => {
			...
			setTimeout(() => this.connectWebSocket(), 5001);
		};
	...
	getMessageHistory(username) {
		return new Promise((resolve, reject) => {
			...
			const send_data = `data=${encodeURIComponent(
	...
	handleIncomingMessage(data) {
		...
		if (data.panelReply !== undefined) {
			...
			// 更新用户列表
			const userExists = vueInstance.userList.some(
				(user) => user[1] === data.panelReply.username
			);
			if (!userExists) {
				vueInstance.userList.push([
					data.panelReply.uid,
					data.panelReply.username,
				]);
			}

			if (
				vueInstance.selectedUser &&
				data.panelReply.username === vueInstance.selectedUser[1]
			) {
				// 处理content_id为0的消息
				if (data.panelReply.id === 0) {
					...
					if (lastIndex >= 0) {
						...
						if (lastMessage.id === 0) {
							lastMessage.content =
								lastMessage.content + data.panelReply.content;
...
new Vue({
	...
	methods: {
		...
		sendMessage() {
			...
			let usernameToSend =
				_this.selectedUser === "others"
					? _this.othersUser
					: _this.selectedUser[1];
			...
			this.fayService.getUserList().then((response) => {
				if (response && response.list) {
					if (response.list.length == 0) {
						// 检查是否已经有默认用户
						const defaultUserExists = this.userList.some(
							(user) => user[1] === "User"
						);
		...
		adoptText(id) {
			// 调用采纳接口
			this.fayService
				.fetchData(`${this.base_url}/api/adopt-msg`, {
					method: "POST",
					headers: {
						"Content-Type": "application/json",
					},
			...
			fetch(mcpUrl, { signal: controller.signal })
				...
				.then((data) => {
					if (Array.isArray(data)) {
						// 检查是否有任何一个MCP服务器在线
						const hasOnlineServer = data.some(
