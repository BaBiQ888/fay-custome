...
def ensure_fay_service_running():
    """确保Fay服务正在运行，如果没有则自动启动"""
    if fay_booter.feiFei is None:
        if not fay_booter.is_running():
            util.log(1, "检测到Fay服务未启动，正在自动启动...")
            from scheduler.thread_manager import MyThread
            ...
            for i in range(30):  # 最多等待30秒
                if fay_booter.feiFei is not None:
                    util.log(1, "Fay服务启动成功")
                    return True
                time.sleep(1)

            util.log(1, "Fay服务启动失败")
            return False
        else:
            util.log(1, "Fay服务正在启动中...")
            return False
    return True
...
def api_send():
    ...
    try:
        ...
        util.printInfo(1, username, '[文字发送按钮]{}'.format(
...
def api_get_Msg():
    try:
        ...
        while i >= 0:
            ...
            timetext = datetime.datetime.fromtimestamp(
                list[i][3], timezone).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            relist.append(dict(type=list[i][0], way=list[i][1], content=list[i][2], createtime=list[i]
                          [3], timetext=timetext, username=list[i][5], id=list[i][6], is_adopted=list[i][7]))
            i -= 1
        if fay_booter.is_running():
            wsa_server.get_web_instance().add_cmd({"liveState": 1})
...
@__app.route('/v1/chat/completions', methods=['POST', 'OPTIONS'])
@__app.route('/api/send/v1/chat/completions', methods=['POST', 'OPTIONS'])
...
def to_greet():
    ...
    observation = data.get('observation', '')
    interact = Interact("hello", 1, {
                        'user': username, 'msg': '按观测要求打个招呼', 'observation': str(observation)})
...
def to_stop_talking():
    try:
        ...
        observation = data.get('observation', '')
        interact = Interact("stop_talking", 2, {
...
def transparent_pass():
    try:
        ...
        util.printInfo(1, user, '透传播放：{}，{}'.format(
...
def api_start_genagents():
    try:
        ...
        instruction_file = os.path.join(os.path.dirname(
            os.path.abspath(__file__)), '..', 'genagents', 'instruction.json')
